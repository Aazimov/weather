...
const MainFooter = ({fiveDays,date, setDate, isLight}) => {
    const {title, listDate, itemDate, row, card, cardTitle, cardImg, cardTempMax, cardTempMin, description, itemDateActive, light} = styles;
                ...
                <ul className={listDate}>

                    {
                       [...  new Set(
                           ...
                           <li key={elem} className={`${itemDate} ${date===elem ? itemDateActive : ''} ${isLight ? light : ''}`}
